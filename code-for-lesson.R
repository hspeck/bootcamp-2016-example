...
# use a t-test:
ttest<-t.test(setosa$Sepal.Length, virginica$Sepal.Length) # this will display the result in the command line
ttest$p.value
...
windows()
par(mar=c(4,4,3,2))
boxplot(setosa$Sepal.Width, virginica$Sepal.Width, names=c('Setosa', "Virginica"), ylab="Sepal Width", xlab="Species", col1=4, col2=3, las=1, cex.axis=1.5,cex.lab=1.5)
###############
...
legend(2,10, # set the location x, y 
		unique(iris$Species), # text to display, calls the text in species name
...
windows()
plot(iris$Petal.Length ~ iris$Sepal.Width + iris$Species -1, data=iris)
#abline can addregression line to the thing, we can copy paste it and change quick but can usually do this with a for loop 
