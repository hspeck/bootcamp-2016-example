...
#this is summing up all nonmissing data occurences (counting the minor alleles) and dividing it but the total number of alleles
freq
...
#another function,calcs the heterozygoisty, aka when an observance is 1, not 2 or 0
calc_het=function(x){
	return(sum(x==1,na.rm=TRUE)/(sum(!is.na(x))))
}

# And now let's apply the functions to each and every SNP, stops us going rowby row
#storing output in freq vector, will give us frequency of each SNP
...
#p is vector of elements 0-.5 by 0.05, and using hardy weinburge equilibrium, type "l" means make it a line
...
#expcnts is expected frequency, and then we put in a vector
#chisq for observed vs expected genotypes
...
#this applies to matrix of snps, rows, and storing in this variable compute_chisquare
...
#mean gives proportion, not a count, aka close to pvalues

#which SNPs?, aka finding the snps with pvalue less than .05, use function subset, this one will pull out whole row
#names function pulls out all the names of the the sig_snps
...
#this pulls out all rows associated with these IDs
